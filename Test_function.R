...
gensamdistrpwdcov <- function(N, dim_list, d = length(dim_list), niter=15000,
                              fixgrid = halton(N, sum(dim_list))){
  # create the combination
  list_combn <- combn(1:length(dim_list), 2)
  fixgrid_list <- list()
  for (k in 1:d) {
    if(k==1){
      fixgrid_list[[k]] <- as.matrix(fixgrid[, (1:dim_list[k])])
  ...
  for(i in 1:niter)
  {
    ...
    for (m in 1:nrow(list_combn)) {
      tstat[i] <- tstat[i] + energy::dcov(per_list[[list_combn[1,m]]], 
                                          per_list[[list_combn[2,m]]], index = 2)
    }
  }
  return(tstat)
}

computestatisticpwrdcov <- function(X, dim_list, n = nrow(X), d = length(dim_list),
                                  gridch = halton(n, sum(dim_list)))
{
  data_list <- list()
  X <- as.matrix(X)
  ...
  randcovSTAT = pwdov(data_list, dim_list = dim_list)
  return(randcovSTAT)
}

pwdov <- function(X, dim_list){
  # create the combination
  list_combn <- combn(1:length(dim_list), 2)
  tstat = 0
  for (m in 1:nrow(list_combn)) {
    tstat <- tstat + energy::dcov(X[[list_combn[1,m]]], 
                                  X[[list_combn[2,m]]], index = 2)
  }
  tstat
}

boot_pwdcov <- function(X, B, dim_list){
  test_stat <- pwdov(X, dim_list = dim_list)
  boot <- numeric(B)
  n <- nrow(X[[1]])
  for (b in 1:B) {
    boot[b] <- pwdov(boot_sample_list(X, n, d = length(X)), dim_list = dim_list)
  }
  length(which(boot >= test_stat)) / (B + 1)
}
